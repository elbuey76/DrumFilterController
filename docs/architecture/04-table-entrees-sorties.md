...
| COM-001 | `IHM_ECRAN` | Must | Banc I2C initial validé le 2026-07-23 | LCD 2004 / 20x4 I2C 3,3 V, fond bleu, module I2C type PCF8574 ou equivalent, raccordement sur `GPIO32` / `GPIO33` de la KC868-A16 via un bus I2C auxiliaire separe | Le LCD recu repond a `0x27`, est lisible et affiche l'heure RTC sur le banc partage avec la DS3231. SDA/SCL sont mesures a 3,287 V au repos, sans pull-up externe. La validation de toutes les pages IHM et un essai prolonge restent a faire. |
| COM-002 | `RTC_DS3231` | Must | Banc I2C initial validé le 2026-07-23 | Module RTC DS3231 I2C 3,3 V avec batterie rechargeable, adresse `0x68`, raccordement sur `GPIO32` / `GPIO33` de la KC868-A16 via le bus I2C auxiliaire partage avec le LCD | La RTC recue repond a `0x68`; l'heure et les diagnostics sont fonctionnels, seule puis avec le LCD. Le Wi-Fi/NTP peut recaler la RTC en V2 mais ne doit pas etre l'unique source. Les temporisations de securite et de cycle ne doivent pas dependre de la RTC. |
...
| Protection pompe rinçage | Disjoncteur 6 A courbe C et contacteur Schneider LC1D18P7 | Coupé ou défaut selon architecture retenue | Pompe classe I à raccorder à la terre, courant nominal 3,5 A ; la courbe C tient compte de l'appel moteur. Le contacteur plus calibre ne change pas le calibre de protection. |
...
| --- | --- | --- |
| Protection amont cote maison | Disjoncteur dedie 16 A | Liaison d'environ 20 m entre tableau maison et coffret filtration, confirmee en 3G2,5 mm2. Le cable est en attente et le disjoncteur 16 A doit etre ajoute au tableau maison. |
...
| Pompe de rincage | Disjoncteur 6 A courbe C | Alimente le depart pompe commande par contacteur Schneider. |
