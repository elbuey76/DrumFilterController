...
| --- | --- | --- | --- | --- | --- |
| DI-001 | `EP_LAVAGE` | Must | Retenu, position a regler | CR18-8DN, NPN, 12-24 VDC, 3 fils | Premier niveau cote eau propre sur le report 32 mm. Demande de lavage lorsque le niveau eau propre est abaisse. Le signal doit etre filtre par retard configurable avant lancement d'un cycle. |
| DI-002 | `EP_CRITIQUE` | Must | Retenu, position a regler | CR18-8DN, NPN, 12-24 VDC, 3 fils | Deuxieme niveau cote eau propre sur le report 32 mm. Force la mise en securite : arret filtration, arret UV, inhibition tambour et rincage. |
...
| AI-002 | `TEMP_LOCAL` | Must | A choisir | Sonde temperature a definir | Mesure la temperature de l'air du local de filtration, avec detection de perte de mesure et seuils d'alerte. |
...
| AI-003 | `COURANT_TAMBOUR` | Should | A definir | Capteur de courant DC ou module de protection avec retour defaut | Permettrait de detecter surintensite, blocage ou effort anormal du moteur tambour. Peut etre remplace par une protection materielle sans retour automate si le diagnostic reste indirect. |

## Sorties automate et puissance

| ID | Nom | Priorite | Etat | Interface cible | Comportement attendu |
| --- | --- | --- | --- | --- | --- |
| DO-001 | `CMD_TAMBOUR` | Must | A dimensionner | Commande 12 V DC petite vitesse du moteur SWF 403.835 via relais, contacteur DC ou module adapte | Commande rotation tambour pendant lavage, test, manuel autorise ou indexation. Inhibee en niveau critique, capot ouvert, maintenance ou defaut critique. |
...
| Protection moteur tambour | Fusible 10 a 15 A initial, limitation ou detection surintensite | Coupe ou defaut selon architecture retenue | Calibre a ajuster apres mesures du courant reel, notamment demarrage et blocage controle. |
...
- Identifier le brochage exact du moteur tambour 5 broches avant de figer la commande petite vitesse et l'usage eventuel du contact parking.
- Choisir si la protection surintensite du moteur tambour remonte une information automate ou reste une protection materielle autonome.
