...
| F-112 | Must | Revue documentaire + Test sur banc | Plateforme compatible heure fiable V2 | Le choix de plateforme V1 documente la RTC DS3231 I2C 3,3 V avec batterie rechargeable comme source locale d'heure fiable V2 ; le banc confirme adresse `0x68`, conservation de l'heure après coupure, pull-up I2C 3,3 V et absence de dépendance exclusive à Internet. | À définir |
| F-113 | Must | Test | Reprise A15 après coupure | Si A15 était actif avant coupure et que le capot reste ouvert au redémarrage, A15 est réaffiché après lecture stable ; si A15 n'était pas actif, la temporisation A15 repart. | Passé (host-side) |
| F-114 | Must | Test | Événements persistants V1 | Le mini-journal persistant conserve au minimum A15, EP_CRITIQUE, capot ouvert dangereux, lavage inefficace et redémarrage après coupure ; les alertes température ne sont pas persistantes en MVP. | Passé (host-side) |
| F-115 | Must | Test | Format mini-journal V1 | Le mini-journal V1 expose des compteurs persistants par code d'événement plus le dernier événement global, ou une mémoire circulaire courte 8/16 événements si retenue par la plateforme. | Passé (host-side) |
| F-116 | Must | Test | Journalisation acquittements | Les acquittements réussis des alarmes bloquantes sont persistants ; les refus répétitifs ne créent pas chacun une entrée persistante et le dernier refus reste affiché localement. | Passé (host-side) |
...
| B-107 | Must | Test sur banc + Inspection | Interface KC868-A16 / CR18-8DN | Les capteurs CR18-8DN alimentés en 12 VDC sont lus correctement sur les entrées digitales KC868-A16 en repos et détection ; le câblage marron +12 VDC, bleu 0 V commun, noir `Xn` (alias firmware `In`) est conforme ; le courant d'entree et les cas fil noir, bleu ou marron débranché sont observés et documentés avec le sens logique firmware retenu. | À définir |
...
| Interface électrique niveau | B-107 | Vérifier sur banc le raccordement direct CR18-8DN vers entrées optocouplées KC868-A16, le courant d'entree, le sens logique lu par le firmware et les comportements de rupture des fils marron, bleu et noir. |
...
| Alertes température V1 | F-031, F-033, F-125, A06, A07, A08, A09, A11, A12 | Vérifier les deux sondes DS18B20 identiques, le brochage réel des fils, l'identification eau/local, les seuils eau/local, la perte de mesure, l'affichage IHM, l'absence de blocage automatique et l'absence de mode hiver automatique en V1. |
