...
| SIM-007 | Pendant `WASH_AUTO`, executer `capot open`. | `FAULT`, alarme `A03`, tambour/rincage `OFF`; reset refuse tant que le capot reste ouvert. | Host-side : `test_sim_007_capot_open_during_wash_triggers_a03_and_blocks_reset` |
...
| SIM-009 | `capot open`, puis `test`. | `A13 - TEST REFUSE CAPOT`, aucune sortie dangereuse active. | Host-side : `test_sim_009_test_refused_when_capot_open` |
...
| SIM-013 | Pendant commande manuelle active, `capot open`. | `FAULT`, alarme `A03`, tambour/rincage `OFF`. | Host-side : `test_sim_013_capot_open_during_manual_command_triggers_a03` |
